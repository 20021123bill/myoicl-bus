...
def validate_episodic(model, make_val_iter, device, charset, n_episodes: int,
    ...
    """Episodic validation on held-out META-VAL users, reporting mode A and
    mode B on the *same* episodes.

    Why this replaces plain windowed validation: with a frozen backbone and no
    context, mode A is mathematically constant --- it is exactly the published
    model --- so a mode-A validation curve carries no information and cannot
    select a checkpoint. What we are training is the conditioned path, so
    validation must supply context. Reporting A and B on identical episodes
    also gives the quantity we actually care about, the conditioning gain.
    """
    ...
    for i, ep in enumerate(make_val_iter()):
        ...
        # mode B (with context) -- what we train
        lossB, emB, lenB = episode_forward(model, ep, device, autocast_dtype)
        losses.append(float(lossB))
        # mode A (same episode, context withheld) -- the frozen reference
        epA = dict(ep)
        epA["ctx_raw"] = None
        epA["ctx_labeled_raw"] = None
        epA["ctx_labeled_ids"] = None
        _, emA, lenA = episode_forward(model, epA, device, autocast_dtype)

        targets = ep["targets"].cpu().numpy()
        tlens = ep["target_lengths"].cpu().numpy()
        for acc, em, ln in ((accB, emB, lenB), (accA, emA, lenA)):
    ...
    return accB.cer, accA.cer, float(np.mean(losses)) if losses else float("nan")
...
def main():
    ...
    if phase == "a":
        ...
        # Deterministic mode-B validation episodes on the meta-val users.
        ...
        def make_val_iter(_bu=by_user_val, _e=ecfg, _d=dcfg, _raw=raw_mode):
            ...
            ds = EpisodeIterableDataset(
                ...
                mode_probs=(0.0, 1.0, 0.0),          # mode B only
    ...
    for step in range(start_step, max_steps):
        ...
        if (step + 1) % val_every == 0:
            if phase == "icl":
                cer, cerA, vloss = validate_episodic(
                    model, make_val_iter, device, cs, val_batches,
                    autocast_dtype, frozen_backbone,
                )
                gain = cerA - cer
                print(f"[val] step {step + 1}: mode-B CER {cer:.2f} | "
                      f"mode-A CER {cerA:.2f} | conditioning gain {gain:+.2f} "
                      f"| loss {vloss:.4f}", flush=True)
                logger.writerow([step + 1, "", "", cer, vloss, f"A={cerA:.2f}"])
            else:
                cer, vloss = validate(
