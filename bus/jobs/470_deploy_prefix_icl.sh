...
exec > >(tee -a "$L/470_prefix_icl.log") 2>&1
...
tar xzf tools/myoicl_prefix_icl.tar.gz -C . || { echo "TAR FAILED"; exit 1; }
...
cs = charset_fn()
enc = PrefixContextEncoder(128, cs.num_classes, sig_stride=2, max_prefix=4096)
...
enc = PrefixContextEncoder(trunk.d_model, cs.num_classes, sig_stride=2)
...
echo "=== 470 done: prefix ICL trainer deployed, waiting on tf_fold0 ==="
