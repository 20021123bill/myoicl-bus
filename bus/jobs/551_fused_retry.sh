...
exec > >(tee -a "$L/551_fused.log") 2>&1
...
CUDA_VISIBLE_DEVICES=3 nohup python -m myoicl.train_prefix_icl \
  ...
  --out-dir "$R/icl_fusedb_fold2" \
  --fused-prefix \
  --max-steps 12000 --val-every 500 --val-episodes 24 \
  --p-synth 0.5 --p-permute 0.5 --p-modeA 0.2 \
  --lr 3e-4 --trunk-lr-mult 0.1 \
  > "$L/icl_fusedb_fold2.log" 2>&1 &
echo "launched icl_fusedb_fold2 pid=$!"

sleep 240
grep -vE "Warning|warn" "$L/icl_fusedb_fold2.log" | head -12

for k in $(seq 1 144); do
  sleep 300
  cp -f "$L/icl_fusedb_fold2.log" bus/results/ 2>/dev/null
  v=$(grep -E "^\[val\]|^\[audit\]|FATAL|Traceback" "$L/icl_fusedb_fold2.log" | tail -1)
  echo "[$(date +%H:%M)] ${v:-running}"
  pgrep -f "icl_fusedb_fold2" >/dev/null || { echo "fused run ended"; break; }
done
echo "=== 551 done ==="
