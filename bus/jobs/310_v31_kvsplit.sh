...
exec > >(tee -a "$L/310_v31.log") 2>&1

echo "=== deploy v3.1 (key/value split) ==="
tar xzf tools/myoicl_v31.tar.gz -C .
git add -A myoicl && git commit -q -m "v3.1: key/value split (key=signal feature, value=char)" 2>&1 | tail -1 || true

echo "=== smoke (tests kv_split False AND True) ==="
python -m myoicl.smoke_v3 || { echo "SMOKE FAILED -- not launching"; exit 1; }

echo "=== wait for v3-main (GPU2) to finish so we can reuse GPU2 ==="
...
PY
  ...
  [ "$s" -ge 11500 ] && { echo "v3-main reached step $s, GPU2 free soon"; break; }
  sleep 60
done
sleep 30
