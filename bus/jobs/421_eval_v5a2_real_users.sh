...
exec > >(tee -a "$L/421_eval_a2_real.log") 2>&1
...
# WHY THIS EXISTS. Job 420 evaluated A1 on the 8 real users and found gain C
# -1.42 at 16 s and -1.41 at 48 s -- FLAT in K, i.e. the module extracts
# nothing at all from real support, while the same checkpoint gains +38.7 on
# synthetic subjects. A1's synthetic family was the LEAST realistic one we
# had: rotation_choices [0], no tilt, no mixing, no noise -- pure per-channel
# gain. A2 was trained on the calibrated family that DOES include integer
# electrode rotation, spectral tilt, neighbour mixing and noise. If sim-to-real
# depends on how realistic the simulated subject is, A2 should transfer better
# than A1; if A2 is also flat, the synthetic route cannot carry real transfer
# on its own and the fold backbones are the only path.
echo "=== wait for job 420 to release GPU3 ==="
for i in $(seq 1 90); do
  pgrep -f "myoicl.eval_qwerty" >/dev/null 2>&1 || break
  [ $((i % 10)) -eq 0 ] && echo "  waiting ($i) for 420"
  sleep 60
done

CK=$R/v5_a2/best.pt
[ -f "$CK" ] || CK=$R/v5_a2/last.pt
[ -f "$CK" ] || { echo "no v5_a2 checkpoint yet -- aborting"; exit 1; }
...
for k in 12 45; do
  ...
  CUDA_VISIBLE_DEVICES=3 python -m myoicl.eval_qwerty \
    ...
    --out "$R/v5a2_real_k$k.json" 2>&1 | tail -25
...
import json, glob, re
rows = []
for f in sorted(glob.glob('/data2/chenyuxiang/runs/v5a2_real_k*.json'),
