...
exec > >(tee -a "$L/240_v3.log") 2>&1

echo "=== deploy v3 fix (encode_context v3 branch moved before stats block) ==="
tar xzf tools/myoicl_v3_fix.tar.gz -C .
git add -A myoicl && git commit -q -m "v3 fix: encode_context v3 branch before stats deref" 2>&1 | tail -1 || true
