...
exec > >(tee -a "$L/260_v3.log") 2>&1

echo "=== deploy smoke fix (set o_proj nonzero instead of unstable SGD) ==="
tar xzf tools/myoicl_v3_smoke2.tar.gz -C .
git add -A myoicl && git commit -q -m "v3 smoke: open path by setting o_proj, no unstable optimizer" 2>&1 | tail -1 || true
