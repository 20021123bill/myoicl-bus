...
exec > >(tee -a "$L/280_v3.log") 2>&1
pkill -f "qwerty_v3" 2>/dev/null; sleep 5

echo "=== deploy pad fix (pad time axis, not window axis) ==="
tar xzf tools/myoicl_v3_pad.tar.gz -C .
git add -A myoicl && git commit -q -m "v3: safety pad on time axis (concat, not F.pad)" 2>&1 | tail -1 || true
...
PY
  ...
  CUDA_VISIBLE_DEVICES=0 python -m myoicl.eval_qwerty \
    --ckpt /tmp/v3_snap.pt --modes A B C --k 12 --ctx-seconds 30 --bf16 \
