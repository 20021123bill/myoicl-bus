...
exec > >(tee -a "$L/531_gate_final.log") 2>&1

# =============================================================================
# Finish the gate reading. 530 established the monitor artifact (same
# checkpoint, same step: 30 s monitor said ~85, the paper's 5 s windowing says
# ~53) but died after two checkpoints, and it only ever looked at best.pt --
# which the CONTAMINATED monitor selected at step 10-12k, one quarter of
# training. The fully-trained step-40000 models have never been measured.
#
# This job evaluates every run's LAST checkpoint (and ref's best for
# completeness), one python process per checkpoint so a crash cannot take the
# rest down, then the window-length sweep.
# =============================================================================
...
out = '/data2/chenyuxiang/runs/gate_final.json'
...
for spec in \
  "$R/tf_ref_lr1e3/last.pt:ref_last40k" \
  "$R/tf_fold0/last.pt:fold0_last" \
  "$R/tf_fold1/last.pt:fold1_last" \
  "$R/tf_fold2/last.pt:fold2_last" \
  "$R/tf_ref_lr1e3/best.pt:ref_best" \
  "$R/tf_fold2/best.pt:fold2_best" ; do
  p=${spec%%:*}; lab=${spec##*:}
  [ -f "$p" ] && one "$p" "$lab" || echo "  (missing $p)"
done

echo
echo "=== window-length sweep on ref_last40k ==="
for W in 10000 20000 60000; do
  CKPT=$R/tf_ref_lr1e3/last.pt LABEL=sweep_$W WLEN=$W python - <<'PY'
import os, torch
from emg2qwerty.charset import charset as charset_fn
from emg2qwerty.data import LabelData
from myoicl.episodes import build_windowed_dataset, windowed_collate
...
from torch.utils.data import DataLoader
cs = charset_fn(); dev = torch.device("cuda:0")
W = int(os.environ["WLEN"])
sess = load_user_sessions('/data2/chenyuxiang/code/emg2qwerty', 'generic')
ck = torch.load(os.environ["CKPT"], map_location='cpu')
m = build_trunk({"model": {"tf_size": "tiny",
                           "conv_strides": ck["args"].get("conv_strides",
                                                          [5, 2, 2])}},
                num_classes=cs.num_classes).to(dev)
m.load_state_dict(ck["model"]); m.eval()
ds = build_windowed_dataset(sess['test'][:4],
    train=False, window_length=W, padding=(1800, 200), raw=True)
dl = DataLoader(ds, batch_size=8, num_workers=2, collate_fn=windowed_collate)
acc = CERAccumulator()
with torch.no_grad():
    for b in dl:
        raw = b["inputs"].permute(1, 2, 3, 0).flatten(1, 2).to(dev)
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            em = m(raw)
        lens = m.output_length(b["input_lengths"].to(dev))
        for n, p in enumerate(greedy_ctc_decode(em.float(), lens.cpu(),
                                                blank=cs.null_class)):
            tg, tl = b["targets"].numpy(), b["target_lengths"].numpy()
            acc.update(LabelData.from_labels(p).text,
                       LabelData.from_labels(tg[: tl[n], n]).text)
print(f"  window {W/2000:4.0f}s: CER {acc.cer:6.2f}", flush=True)
PY
done
cp -f "$R/gate_final.json" bus/results/archive/ 2>/dev/null
